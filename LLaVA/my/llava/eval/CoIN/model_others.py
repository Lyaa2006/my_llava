...
def shorten_answer(text: str) -> str:
    ...
    text = re.sub(r"^(it is|it's|this is|there is|there are|the image shows|the image is of)\s+", "", text, flags=re.IGNORECASE).strip()
...
def eval_model(args):
    ...
    for line in tqdm(questions):
        ...
        if args.append_instruction:
            qs = f"{qs.rstrip()} {args.append_instruction}".strip()
        ...
        if args.short_answer:
            outputs = shorten_answer(outputs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--append_instruction", type=str, default="")
    parser.add_argument("--short_answer", action="store_true")
